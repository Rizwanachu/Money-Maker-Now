...
export default function ProposalDetailPage() {
  ...
  // Initialize local content when proposal loads
  useEffect(() => {
    if (proposal) {
      const initial: Record<string, string> = {};
      SECTIONS.forEach(sec => {
        if (proposal[sec.id as keyof Proposal] !== undefined) {
          initial[sec.id] = (proposal[sec.id as keyof Proposal] as string) || "";
        }
      });
      setLocalContent(prev => ({ ...initial, ...prev })); // only keep prev if we already started editing
    }
  }, [proposal]);
  ...
  const handleGenerate = async (sectionId: string) => {
    ...
    setLocalContent(prev => ({ ...prev, [sectionId]: "" }));
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (data.content) {
                ...
                setLocalContent(prev => ({ ...prev, [sectionId]: generatedText }));
              }
            } catch (e) {
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <SelectTrigger className={`w-[140px] h-9 ${
                proposal.status === 'accepted' ? 'bg-green-50 text-green-900 border-green-200' :
                proposal.status === 'sent' ? 'bg-blue-50 text-blue-900 border-blue-200' :
                proposal.status === 'declined' ? 'bg-red-50 text-red-900 border-red-200' :
                ''
              }`}>
                <SelectValue />
              </SelectTrigger>
          ...
          <Card className="flex flex-col overflow-hidden h-fit max-h-full">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {SECTIONS.map((section) => {
                const hasContent = !!localContent[section.id];
          ...
          <Card className="flex flex-col min-h-0">
            ...
            <CardContent className="p-0 flex-1 relative flex flex-col">
              <Textarea 
                ...
                placeholder={generating === activeSection 
                  ? "AI is generating content..." 
                  : "Write your content here or use AI to generate a draft..."}
